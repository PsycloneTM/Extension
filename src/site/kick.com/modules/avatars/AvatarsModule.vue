...
async function awaitAvatarElement(): Promise<HTMLElement> {
	const n = document.querySelector<HTMLElement>(".object-cover");
	if (n) {
		...
		(entries, emit) => {
			for (const entry of entries) {
				...
				for (let i = 0; i < entry.addedNodes.length; i++) {
					...
					const n = node.querySelector<HTMLElement>(".object-cover");
					if (!n) continue;
...
async function findAvatarClass() {
	...
	if ((com.return.type as { displayName?: string }).displayName !== "profile-picture") return;
...
function patchImageAvatar(component: Kick.UserAvatar) {
	...
	const fi = avatar.data.host.files.find((f) => f.width && f.width > 80);
	if (!fi) return;
...
function assignAvatar(av: SevenTV.Cosmetic<"AVATAR">) {
	...
	const con = av.data.user.connections.find((con) => con.platform === "KICK");
